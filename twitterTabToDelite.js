// 開いているツイートをデライトに転記して、現在のタブでなければそのタブを閉じる AutoControl script
// background で動作可能
// EXTERNAL_DEPENDENCY: 外部依存です。主にAutoControlのGUI設定
...
for (const [tabId, { title, url, window, active }] of tabsInfo) {
	...
	if (tweetUrl) {
		...
		// const { name, _content } = title.match(tweetTitleRegex).groups
		...
		const dlnProvider = (tw, hasInnerTweet = false) => {
			...
			const dln =
				`>>
${tw.content}${innerMedia}${hasInnerTweet ? `\n...引用` : ""}
...
${media}`;
		...
		// if (!active) tabsToClose.push(tabId)
		if (!active) await ACtl.closeTab(tabId)
	}
}
