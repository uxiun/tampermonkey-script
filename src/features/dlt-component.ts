...
export const candidateTip = (
  ...
  const fgHtml =
    fgTitles.length > 0
      ? `<div style="font-size: ${option?.fgFontSize ?? "14px"}; color: #89b4fa; margin-bottom: 2px; white-space: nowrap; ${isSelected ? "font-weight: bold;" : ""}">
                ${fgTitles.join("｜")}
               </div>`
      : ""
  ...
  return `
          <div style="flex: 0 1 auto; min-width: 0; overflow: hidden; padding: 5px 7px; background: ${bg}; border: ${border}; border-radius: 6px; box-shadow: ${boxShadow}; backdrop-filter: blur(4px); transition: all 0.08s ease; max-width: 100%;">
            ${fgHtml}
            <div style="color: #cdd6f4; white-space: nowrap; text-overflow: ellipsis; overflow: hidden; font-size: ${option?.titleFontSize ?? "17px"};">
            ${
              withId
